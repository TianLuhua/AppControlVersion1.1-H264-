...
public class MainActivity extends Activity implements OnClickListener {
			...
			if (back != null && back.startsWith("serverip:")) {
				...
				if (!hasDeviceInfo(deviceInfos, serverIp)) {
					...
					DatagramPacket packet = new DatagramPacket(over,
							over.length, broadcastAddress, MULTIPORT);
					multicastSocket.send(packet);
					mHandler.sendEmptyMessageDelayed(SCAN_IP_OVER, 2000);

				}
				Log.i(TAG, "hdb-------serverIp:" + serverIp + "   split[2]:"
						+ split[2]);
	...
	public void initDecoder() {
		try {
			...
			// 64],PPS=[104, -50, 6, -30
			// [0, 0, 0, 1, 103, 66, -64, 41, -115, 104, 4, 0, 77, 121,96, 30,
			// 17, 8, -44, 0, 0, 0, 1, 104, -50, 1, -88, 53, -56
