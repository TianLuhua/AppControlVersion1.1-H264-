...
import com.sat.mirrorcontorl.utils.EventInput;
...
public class MediaCodeUdpService extends Service implements
			...
			public void run() {
				try {
					...
					while (isRun) {
						...
						Log.i(TAG, "hdb-----lenght:" + pack.getLength()
								+ "   offset:" + pack.getOffset());
						String point = new String(pack.getData(),
	...
	public void onImageData(byte[] buf) {
		...
		if (null != dSocketData) {
			try {
				/*
				 * byte[] bytes = new byte[buf.length + 4]; byte[] head =
				 * intToBuffer(buf.length); System.arraycopy(head, 0, bytes, 0,
				 * head.length); System.arraycopy(buf, 0, bytes, head.length,
				 * buf.length); os.write(bytes);
				 */
