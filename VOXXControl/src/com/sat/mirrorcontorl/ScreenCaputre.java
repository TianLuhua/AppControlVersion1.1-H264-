...
public class ScreenCaputre {
	...
	// setVideoSmoothMode
	private final static int FRAME_BIT_RATE = (int) (1024 * 1024 * 0.5);
	...
	public void prepareVideoEncoder() throws IOException {
		...
		vEncoder = MediaCodec.createEncoderByType(MIMETYPE_VIDEO_AVC);
	...
	public void startVideoEncode() {
		if (vEncoder == null) {
			throw new RuntimeException("pl Init MediaCodec");
		}
		if (videoEncoderLoop) {
			throw new RuntimeException("pl stop");
		}
		videoEncoderThread = new Thread() {
			...
			public void run() {
				...
				vEncoder.start();
				ByteBuffer[] encoderOutputBuffers = vEncoder.getOutputBuffers();
				while (videoEncoderLoop && !Thread.interrupted()) {
					try {
						int encoderStatus = vEncoder.dequeueOutputBuffer(
								vBufferInfo, TIMEOUT_USEC);
						if (encoderStatus >= 0) {
							ByteBuffer bb = encoderOutputBuffers[encoderStatus];
